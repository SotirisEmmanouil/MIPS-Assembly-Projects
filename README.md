...
This program receives an input and if the input is a floating point number, the program truncates it so that it becomes an integer by manually shifting the bits around, essentially reversing the process of turning an integer in binary to the IEE 754 Floating point format. Once the number has been truncated, the program will add all the integers between 1 and itself, printing the solution. This project was completed while taking Professor Frary's CSCI240 Computer Organization and Assembly Language course at CUNY Queens College during the Spring 2022 semester
